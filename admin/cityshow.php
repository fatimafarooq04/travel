...
// Get the total number of entries in the city table
$total_entries_query = "SELECT COUNT(*) AS total FROM `city`";
...
$qry = "SELECT * FROM `city` LIMIT $start, $entries_per_page";
...
<div class="container">
    <h1 class="mt-5 text-center">City</h1>
        ...
        <tbody>
            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                <tr>
                    ...
                    <td>
                        <a href="cityedit.php?updid=<?php echo $row['CityID'] ?>" class="btn btn-sm btn-info mx-2" title="Edit"><i class='fas fa-pencil-alt'></i> Edit</a>
                        <a href="citydelete.php?dltid=<?php echo $row['CityID'] ?>" class="btn btn-sm btn-danger" title="Delete"><i class='fas fa-trash-alt'></i> Delete</a>
                    </td>
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('input', function () {
        ...
        rows.forEach(function (row) {
            var cityName = row.cells[1].textContent.toLowerCase().trim();
