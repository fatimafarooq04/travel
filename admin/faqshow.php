...
$total_entries_query = "SELECT COUNT(*) AS total FROM `faqs`";
...
$query = "SELECT `faq_id`, `faq_ques`, `faq_ans` FROM `faqs` LIMIT $start, $entries_per_page";
...
<div class="container">
    <h1 class="mt-5 text-center">FAQs List</h1>
        ...
        <tbody>
            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                <tr>
                    ...
                    <td><?php echo $row['faq_ques'] ?></td>
                    <td><?php echo $row['faq_ans'] ?></td>
                
                    <td>
                        <a href="faqedit.php?updid=<?php echo $row['faq_id'] ?>" title="View Details">
                        <button class='btn btn-warning btn-sm'>Edit</button>
                        </a>
                        <a href="faqdelete.php?id=<?php echo $row['faq_id'] ?>" title="View Details">
