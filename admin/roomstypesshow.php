...
// Get the total number of entries in the room_types table
$total_entries_query = "SELECT COUNT(*) AS total FROM `room_types`";
...
$qry = "SELECT * FROM `room_types` LIMIT $start, $entries_per_page";
...
<div class="container">
    <h1 class="mt-5 text-center">Room_Types</h1>
    ...
    <table class="table" id="roomTypesTable">
        <thead>
            <tr>
                <th scope="col">#</th>
                <th scope="col">Room_Types Name</th>
        ...
        <tbody>
            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                <tr>
                    <th scope="row"><?php echo $serial_num ?></th>
                    <td><?php echo $row['TypeName'] ?></td>
                    <td>
                        <a href="roomtypeedit.php?updid=<?php echo $row['RoomTypeID'] ?>" class="btn btn-sm btn-info mx-2" title="Edit"><i class='fas fa-pencil-alt'></i> Edit</a>
                        <a href="roomtypedelete.php?id=<?php echo $row['RoomTypeID'] ?>" class="btn btn-sm btn-danger" title="Delete"><i class='fas fa-trash-alt'></i> Delete</a>
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('input', function () {
        ...
        var rows = document.querySelectorAll('#roomTypesTable tbody tr');

        rows.forEach(function (row) {
            var typeName = row.cells[1].textContent.toLowerCase().trim();
