...
$total_entries_query = "SELECT COUNT(*) AS total FROM `news`";
...
$qry = "SELECT * FROM `news` LIMIT $start, $entries_per_page";
...
<div class="container">
    <h1 class="mt-5 text-center">News Letter</h1>
        ...
        <tbody>
            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                <tr>
                    ...
                    <td><?php echo $row['new_mail'] ?></td>
                    <td><?php echo $row['new_register'] ?></td>
                    <td>
                        <a href="newsdelete.php?dltid=<?php echo $row['new_id'] ?>" class="btn btn-sm btn-danger" title="Delete"><i class='fas fa-trash-alt'></i> Delete</a>
                    </td>
                </tr>
                <?php
                $serial_num++;
