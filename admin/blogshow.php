...
$total_entries_query = "SELECT COUNT(*) AS total FROM `blog`";
...
$query = "SELECT `blog_id`, `blog_date`, `blog_month`, `blog_head`, `blog_img` FROM `blog` LIMIT $start, $entries_per_page";
...
<div class="container">
    <h1 class="mt-5 text-center">Blog List</h1>
        ...
        <tbody>
            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                <tr>
                    ...
                    <td><?php echo $row['blog_month'] ?></td>
                    <td><?php echo $row['blog_head'] ?></td>
                    <td>
                        <?php
                        $imagePath = $row['blog_img'];
                        if (!empty($imagePath)) {
                            echo '<img src="' . $imagePath . '" alt="Blog Image" style="max-width: 100px; max-height: 100px;">';
                        } else {
                            echo 'No Image';
                        }
                        ?>
                    ...
                    <td>
                        <a href="blogdetails.php?id=<?php echo $row['blog_id'] ?>" title="View Details">
