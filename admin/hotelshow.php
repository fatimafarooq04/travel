...
// Get the total number of entries in the hotel table
$total_entries_query = "SELECT COUNT(*) AS total FROM `hotel`";
...
// Fetch the entries for the current page with city names joined
$qry = "SELECT h.*, c.CityName 
        FROM `hotel` h 
...
if (!$result) {
    die("Error executing qry: " . mysqli_error($conn));
}
...
<div class="container">
    <h1 class="mt-5 text-center">Hotels List</h1>
        ...
        <tbody>
            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                <tr>
                    ...
                    <td><?php echo $row['CityName'] ?></td> <!-- Display City Name -->
                    <td>
                        <?php
                        $imagePath = $row['CardImage'];
                        if (!empty($imagePath)) {
                            echo '<img src="' . $imagePath . '" alt="Hotel Image" style="max-width: 100px; max-height: 100px;">';
                        } else {
                            echo 'No Image';
                        }
                        ?>
                    ...
                    <td>
                        <a href="hoteldetails.php?id=<?php echo $row['HotelID'] ?>" title="View Details">
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('input', function () {
        ...
        var rows = document.querySelectorAll('#hotelsTable tbody tr');

        rows.forEach(function (row) {
            var name = row.cells[1].textContent.toLowerCase().trim();
            var city = row.cells[2].textContent.toLowerCase().trim(); // Search by City Name
