...
// Get the total number of entries in the facility table
$total_entries_query = "SELECT COUNT(*) AS total FROM `facility`";
...
$qry = "SELECT * FROM `facility` LIMIT $start, $entries_per_page";
...
<div class="container">
    <h1 class="mt-5 text-center">Facilities</h1>
        ...
        <tbody>
            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                <tr>
                    ...
                    <td>
                        <a href="facilityedit.php?updid=<?php echo $row['FacilityID'] ?>" class="btn btn-sm btn-info mx-2" title="Edit"><i class='fas fa-pencil-alt'></i> Edit</a>
                        <a href="facilitydelete.php?dltid=<?php echo $row['FacilityID'] ?>" class="btn btn-sm btn-danger" title="Delete"><i class='fas fa-trash-alt'></i> Delete</a>
                    </td>
                </tr>
                <?php
                $serial_num++;
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('input', function () {
        ...
        var rows = document.querySelectorAll('#facilitiesTable tbody tr');

        rows.forEach(function (row) {
            var facilityName = row.cells[1].textContent.toLowerCase().trim();
